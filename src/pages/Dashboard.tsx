    ...
    <div className="flex items-center justify-between">
      <div className="flex items-center">
        <div
          ...
          style={{
            background: colorClass.includes('blue')
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const weightedAccuracy = subjectStats.reduce(
          (acc, s) => acc + (s.correctPercentage || 0) * (s.totalCards || 0),
          0
        );
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h1 className="text-5xl font-bold text-white mb-3">🎯 {tx.dashboard}</h1>
          ...
          {subjects.length > 0 ? (
                  ...
                  <div className="p-8">
                    ...
                    <div className="mb-6">
                      <div className="flex justify-between text-sm mb-3">
                        <span className="text-gray-600 font-medium">
                          {t('subject.successRate')}
                        </span>
                        <span className="font-bold text-gray-800">
                          {Math.round(subject.correctPercentage || 0)}%
                        </span>
                      </div>
                    ...
                    {subject.lastStudied && (
                      <p className="text-sm text-gray-500 mb-6 font-medium">
                        📅 {t('subject.lastStudied')}{' '}
                        {new Date(subject.lastStudied).toLocaleDateString(locale)}
                      </p>
                    )}
                  </div>
            ...
            >
              ...
              <Link
                ...
                style={{ background: 'linear-gradient(135deg, #3b82f6, #1d4ed8)' }}
              >
                <Plus className="w-6 h-6 mr-3" />✨ {tx.createCard}
              </Link>
            </div>
          )}
        </div>
            ...
            <div className="flex items-end space-x-3 h-40 bg-gray-50 rounded-2xl p-4">
              {dailyActivity.map((day) => (
                <div key={day.date} className="flex-1 flex flex-col items-center">
                  ...
                  <span className="text-xs text-gray-500">{day.questions_answered}</span>
                </div>
              ))}
            </div>
          </div>
