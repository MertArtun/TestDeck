...
import { createCard, createMultipleCards } from '@/services/database';
      ...
      const cards: Omit<Card, 'id' | 'created_at' | 'updated_at'>[] = questions.map((q, index) => {
        ...
        if (q.options && Array.isArray(q.options)) {
          ...
          if (optionA && optionB && optionC && optionD) {
            ...
            throw new Error(
              `Soru ${index + 1}: Seçenekler bulunamadı. 'options', 'choices', 'answers' dizisi veya 'A', 'B', 'C', 'D' alanları gerekli.`
            );
          }
        }
        ...
        if (typeof correctValue === 'number') {
          ...
          if (!isNaN(parsed)) {
            ...
            if (['A', 'B', 'C', 'D', 'E'].includes(letter)) {
              ...
              throw new Error(
                `Soru ${index + 1}: Doğru cevap formatı geçersiz. '${correctValue}' desteklenmiyor.`
              );
            }
          }
        } else {
        ...
        if (correctIndex < 0 || correctIndex >= options.length) {
          throw new Error(
            `Soru ${index + 1}: Doğru cevap indeksi geçersiz. ${correctIndex} >= ${options.length}`
          );
        }

        const correctAnswerLetter = ['A', 'B', 'C', 'D', 'E'][correctIndex] as
          | 'A'
          | 'B'
          | 'C'
          | 'D'
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="flex gap-6">
                ...
                >
                  <div className="text-center">
                    <div
                      className={`text-xl font-bold mb-2 ${questionType === 'multiple_choice' ? 'text-white' : 'text-gray-800'}`}
                    >
                      {t('create.multipleChoice')}
                    </div>
                    <div
                      className={`text-sm ${questionType === 'multiple_choice' ? 'text-white/90' : 'text-gray-600'}`}
                    >
                      {t('create.multipleChoice.desc')}
                    </div>
                  </div>
                </button>
                ...
                >
                  <div className="text-center">
                    <div
                      className={`text-xl font-bold mb-2 ${questionType === 'fill_in_blank' ? 'text-white' : 'text-gray-800'}`}
                    >
                      {t('create.fillInBlank')}
                    </div>
                    <div
                      className={`text-sm ${questionType === 'fill_in_blank' ? 'text-white/90' : 'text-gray-600'}`}
                    >
                      {t('create.fillInBlank.desc')}
                    </div>
                  </div>
                </button>
              </div>
            ...
            <div>
              ...
              {questionType === 'fill_in_blank' && (
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  {t('create.answerHint')}{' '}
                  <code className="bg-gray-100 px-1 dark:bg-gray-700">_____</code>
                </p>
              )}
            </div>
            ...
            {questionType === 'fill_in_blank' && (
              <div>
                ...
                />
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  💡 İpucu: Birden fazla doğru cevap varsa virgülle ayırın (örn:
