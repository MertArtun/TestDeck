...
import { getSubjectStats, getDailyStats, getAllCards } from '@/services/database';
...
const Statistics = () => {
  ...
  const accuracyDistribution = [
    {
      name: 'Mükemmel (90-100%)',
      value: subjects.filter((s) => (s.accuracy || 0) >= 90).length,
      color: '#10b981',
    },
    {
      name: 'İyi (70-89%)',
      value: subjects.filter((s) => (s.accuracy || 0) >= 70 && (s.accuracy || 0) < 90).length,
      color: '#3b82f6',
    },
    {
      name: 'Orta (50-69%)',
      value: subjects.filter((s) => (s.accuracy || 0) >= 50 && (s.accuracy || 0) < 70).length,
      color: '#f59e0b',
    },
    {
      name: 'Düşük (<50%)',
      value: subjects.filter((s) => (s.accuracy || 0) < 50).length,
      color: '#ef4444',
    },
  ...
  const StatCard = ({
    ...
    >
      <div className="flex items-center">
        <div className="p-5 rounded-2xl text-white mr-6 shadow-lg" style={{ background: color }}>
