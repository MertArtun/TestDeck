...
// Validate card data
function validateCard(card: Record<string, unknown>): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
...
function normalizeCard(card: Record<string, unknown>): ParsedCard {
...
function parseJSON(content: string): ImportResult {
  ...
  try {
    ...
    let cardsArray: Record<string, unknown>[] = [];
...
function parseXML(content: string): ImportResult {
  ...
  try {
    ...
    if (cardElements.length === 0) {
      result.errors.push(
        "XML'de kart bulunamadı. <card>, <question>, <item> veya <q> etiketleri bekleniyor."
      );
      return result;
    }
    ...
    cardElements.forEach((element, index) => {
      try {
        ...
        card.option_a =
          getElementText('option_a') || getElementText('a') || getElementText('optionA');
        card.option_b =
          getElementText('option_b') || getElementText('b') || getElementText('optionB');
        card.option_c =
          getElementText('option_c') || getElementText('c') || getElementText('optionC');
        card.option_d =
          getElementText('option_d') || getElementText('d') || getElementText('optionD');
        card.option_e =
          getElementText('option_e') || getElementText('e') || getElementText('optionE');
        card.correct_answer =
          getElementText('correct_answer') || getElementText('answer') || getElementText('correct');
        card.subject =
          getElementText('subject') || getElementText('category') || getElementText('topic');
        ...
        if (!card.option_a && !card.option_b && !card.option_c && !card.option_d) {
          const combinedText =
            getElementText('options') || getElementText('choices') || card.question;
          if (combinedText) {
      ...
      try {
        ...
        if (values.length !== headers.length) {
          result.warnings.push(
            `Satır ${i + 1}: Sütun sayısı uyumsuz (${values.length} vs ${headers.length})`
          );
    ...
    switch (extension) {
      ...
      default:
        return {
          ...
          cards: [],
          errors: [
            `Desteklenmeyen dosya formatı: ${extension}. Desteklenen formatlar: JSON, XML, CSV`,
          ],
          warnings: [],
          totalProcessed: 0,
        };
